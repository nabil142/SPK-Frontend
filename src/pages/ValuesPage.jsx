    ...
    try {
      const promises = alts.map(a => {
        ...
        criteria.forEach(c => {
          ...
          if (valStr !== undefined && valStr !== '') {
            rowValues.push({ criteria_id: c.criteria_id, value: parseFloat(valStr) })
          }
        })
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            {saveStatus && (
              <span className={`text-xs px-2 py-1 rounded-full ${
                saveStatus.includes('Gagal') ? 'bg-red-500/20 text-red-400' : 'bg-emerald-500/10 text-emerald-400'
              }`}>
          ...
          <p className="text-slate-500 text-sm mt-1">Isi nilai setiap alternatif terhadap masing-masing kriteria. Data tersimpan otomatis.</p>
        </div>
        <div className="flex gap-3">
          <Button variant="secondary" onClick={() => handleNavigate(`/alternatives/${caseId}`)}>
            ← Kembali
          </Button>
          <Button onClick={() => handleNavigate(`/alt-comparison/${caseId}`)}>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-800 flex justify-between items-center">
          ...
          {isLoading && <span className="text-xs text-amber-400 animate-pulse">Memuat data dari server...</span>}
        </div>
        
        <div className="overflow-x-auto">
          {!isLoading && criteria.length === 0 && alts.length === 0 ? (
            <div className="p-8 text-center text-slate-500 text-sm">
              Tidak ada data Kriteria atau Alternatif yang ditemukan untuk kasus ini.
            </div>
          ) : (
                  ...
                  {criteria?.map((c) => (
                    <th key={c.criteria_id} className="px-3 py-3 text-center min-w-[130px]">
                      ...
                      <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${
                        c.criteria_type === 'benefit' ? 'text-emerald-400 bg-emerald-500/10' : 'text-red-400 bg-red-500/10'
                      }`}>
