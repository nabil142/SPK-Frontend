...
import { getCriteria, saveCriteriaComparisons } from '../services/api'
...
function getId(c) { return c.criteria_id ?? c.id }
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="flex-1">
          ...
          <p className="text-center text-xs mt-3 font-medium">
            ...
            <span className="text-slate-400 font-semibold">{pos === 4 ? right.criteria_name : subordinateName}</span>
          </p>
        </div>
...
export default function CriteriaWeightPage() {
  ...
  useEffect(() => {
    ...
    getCriteria(caseId)
      .then(res => { setCriteria(res.data?.data ?? res.data ?? []) })
      .catch(() => {})
  }, [caseId])
  ...
  const autoSaveData = async () => {
    ...
    localStorage.setItem(`ahp_criteria_${caseId}`, JSON.stringify(slidersRef.current))
    ...
    try {
      ...
      // Simpan ke database
      await saveCriteriaComparisons({ case_id: Number(caseId), comparisons, weights: currentWeights })
  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 space-y-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-slate-500 text-sm ml-10">
              Skala ini digunakan AHP untuk menentukan bobot kriteria — data tersimpan otomatis.
            </p>
          </div>
        </div>
          ...
          <div className={`rounded-xl px-4 py-3.5 border flex items-start gap-3 ${crResult.isValid ? 'bg-emerald-500/10 border-emerald-500/25' : 'bg-red-500/10 border-red-500/25'}`}>
            ...
            <div>
              ...
              <p className={`text-xs mt-0.5 ${crResult.isValid ? 'text-emerald-400/70' : 'text-red-400/70'}`}>
                {crResult.isValid ? 'Matriks konsisten (CR ≤ 0.1). Anda bisa melanjutkan.' : 'Matriks tidak konsisten (CR > 0.1). Silakan sesuaikan perbandingan di bawah.'}
              </p>
            </div>
          </div>
        )}
        ...
        {criteria.length < 2 ? (
          <div className="text-center py-12">
            <p className="text-slate-500 text-sm">Tambahkan minimal 2 kriteria terlebih dahulu.</p>
            <Button variant="outline" className="mt-4" onClick={() => handleNavigate(`/criteria/${caseId}`)}>
              ← Kembali ke Kriteria
            </Button>
          </div>
        ) : (
        ...
        {weights.length > 0 && criteria.length >= 2 && (
          <div className="bg-slate-800/60 rounded-xl p-4 border border-slate-700/50">
            <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider mb-3">Preview Bobot Kriteria</p>
            <div className="space-y-2.5">
              {criteria.map((c, i) => (
                <div key={getId(c)} className="flex items-center gap-3">
                  <span className="text-slate-400 text-sm w-36 shrink-0 truncate">{c.criteria_name}</span>
                  <div className="flex-1 bg-slate-700/50 rounded-full h-2 overflow-hidden">
                    <div className="h-full bg-blue-500 rounded-full transition-all duration-500" style={{ width: `${((weights[i] ?? 0) * 100).toFixed(1)}%` }} />
                  </div>
                  <span className="text-blue-400 font-mono text-sm w-14 text-right">{((weights[i] ?? 0) * 100).toFixed(1)}%</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="flex items-center justify-between">
        <Button variant="secondary" onClick={() => handleNavigate(`/criteria/${caseId}`)}>← Kembali</Button>
        <div className="flex gap-3">
          <Button variant="outline" onClick={handleValidate} loading={validating}>Validasi Konsistensi</Button>
          <Button onClick={() => handleNavigate(`/alternatives/${caseId}`)}>Lanjut →</Button>
        </div>
      </div>
    </div>
  )
